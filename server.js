...
async function bootstrap() {
  ...
  server.listen(PORT, () => {
    ...
    console.log(`Storage: ${database.enabled ? "mysql" : "files"}`);
  });
}
...
async function getHealth() {
  ...
  return {
    ...
    docker,
    dataDir: DATA_DIR,
    database: database.enabled ? "mysql" : "files"
  };
}
...
function appendLog(job, message) {
  ...
  job.logsText = (job.logsText || "") + (text.endsWith("\n") ? text : `${text}\n`);
  if (job.logsText.length > 30000) {
    job.logsText = job.logsText.slice(-30000);
  }
  touch(job);
}
...
function publicJob(job) {
  return {
    ...
    error: job.error,
    logs: fs.existsSync(job.logPath) ? tailFile(job.logPath, 30000) : job.logsText || ""
  };
}
...
async function loadDatabaseJobs() {
  ...
  try {
    const dbJobs = await database.loadJobs();
    for (const job of dbJobs) {
      const dir = path.join(JOBS_DIR, job.id);
      jobs.set(job.id, {
        ...job,
        dir,
        inputPath: path.join(dir, "queries.txt"),
        rawPath: path.join(dir, "raw-results.json"),
        logPath: path.join(dir, "run.log"),
        queries: [],
        preview: (job.results || []).slice(0, 100)
      });
    }
  } catch (error) {
    console.warn(`Could not load jobs from MySQL: ${error.message}`);
  }
}
...
function loadExistingJobs() {
  ...
  for (const id of fs.readdirSync(JOBS_DIR)) {
    ...
    jobs.set(id, {
      ...
      results,
      logsText: fs.existsSync(logPath) ? tailFile(logPath, 30000) : "",
      error: null
    });
  }
}
...
function persistJob(job) {
  if (!database.enabled) return;
  database.saveJob(job).catch((error) => {
    console.warn(`Could not save job ${job.id} to MySQL: ${error.message}`);
  });
}

function persistResults(job, rows) {
  if (!database.enabled) return;
  database.saveJob(job)
    .then(() => database.saveResults(job.id, rows))
    .catch((error) => {
      console.warn(`Could not save results for ${job.id} to MySQL: ${error.message}`);
    });
}
...
function sendDownload(res, job, format) {
  ...
  if (!filePath || !fs.existsSync(filePath)) {
    if (job.results?.length) {
      ...
      res.end(format === "json" ? JSON.stringify(job.results, null, 2) : toCsv(job.results));
      return;
    }
